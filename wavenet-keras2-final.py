...
def frame_generator(sr, audio, frame_size, frame_shift, minibatch_size=20):
	...
	while 1:
		for i in range(0, audio_len - frame_size - 1, frame_shift):
			...
			print temp
			target_val = (np.sign(temp) * (np.log(1 + 256*np.abs(temp)) / (np.log(1+256))) + 1)/2.0 * 255
...
class SaveAudioCallback(Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		if (epoch+1)%self.ckpt_freq==0:
			...
			filepath = os.path.join('/home/tushhar/projects/ML/Music/output/', 'ckpt_'+ts+'.wav')
...
if __name__ == '__main__':
	...
	model.fit_generator(training_data_gen, samples_per_epoch=3000, nb_epoch=n_epochs, verbose=1, callbacks=[save_audio_clbk])
	print 'Saving model...'
	...
	outfilepath = '/home/tushhar/projects/ML/Music/output/generated_'+str_timestamp+'.wav'
